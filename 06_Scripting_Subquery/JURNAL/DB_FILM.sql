...
-- TP Week 6 Scripting Subquery --

-- 2. Subquery: Judul film durasi terpendek dan umur termuda yang menonton
SELECT 
    f.judul AS judul_film,
    f.durasi AS durasi_film,
    m.nama AS nama_penonton,
    EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM m.tgl_lahir) AS umur
FROM film_103122400028 f
JOIN jadwal_103122400028 j ON f.id_film = j.id_film
JOIN transaksi_103122400028 tr ON j.id_jadwal = tr.id_jadwal
JOIN member_103122400028 m ON tr.id_member = m.id_member
WHERE f.durasi = (
    SELECT MIN(durasi) FROM film_103122400028
)
AND m.tgl_lahir = (
    SELECT MAX(m2.tgl_lahir)
    FROM member_103122400028 m2
    JOIN transaksi_103122400028 tr2 ON m2.id_member = tr2.id_member
    JOIN jadwal_103122400028 j2 ON tr2.id_jadwal = j2.id_jadwal
    JOIN film_103122400028 f2 ON j2.id_film = f2.id_film
    WHERE f2.durasi = (SELECT MIN(durasi) FROM film_103122400028)
);

-- 3. Subquery + JOIN: Nomor teater, kelas, dan total pemakaian terbanyak
SELECT 
...
HAVING COUNT(tr.kode_pemesanan) = (
    SELECT MAX(jml) FROM (
        SELECT COUNT(tr2.kode_pemesanan) AS jml
        FROM jadwal_103122400028 j2
        JOIN transaksi_103122400028 tr2 ON j2.id_jadwal = tr2.id_jadwal
